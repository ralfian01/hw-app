 ...
 * 
 * @method static mixed getWithProfile() Get account with its profile info
 * @method mixed getWithProfile() Get account with its profile info
 */
class AccountModel extends Model
{
    ...
    /**
     * Profile info from relation between account, and profile
     */
    public function accountProfile()
    {
        return $this->hasOne(ProfileModel::class, 'pa_id');
    }
     ...
     * 
     */
    protected function scopeGetWithProfile(Builder $query)
    {
        return $query
            ->with(['accountProfile'])
            ->addSelect(['pa_id'])
            ...
            ->map(function ($acc) {
                ...
                $acc->makeHidden(['accountProfile', 'pa_id']);

                $acc->name = $acc->accountProfile->ppr_name ?? null;
                $acc->phoneNumber = $acc->accountProfile->ppr_phoneNumber ?? null;
